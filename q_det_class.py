...
class q_lenner():
    ...
    def q_measure(self, M, src, dst):
        ...
        xnew1, ynew1, znew1 = M.dot(np.array(src))
    ...
    def run(self, sframe, visualize=False):
        ...
        try:
            ...
            if self.frame_no == 2:
                M, _ = cv2.findHomography(
                    np.array(self.input_array), np.array(self.dst_array), cv2.LMEDS, 5)
                # print("H matrix:\n", M)
                # scale = 2.5
                M *= self.scale
        ...
        if self.old_frame is not None:
            ...
            for i, win in enumerate(corn_neigh):
                max_val.append(np.max(np.subtract(
                    gr[win[0]:win[1], win[2]:win[3]], self.old_frame[win[0]:win[1], win[2]:win[3]])))
                thresh = 0.5 * \
                    (np.max(gr[win[0]:win[1], win[2]:win[3]]) -
                     np.min(gr[win[0]:win[1], win[2]:win[3]]))
                # print("threshold", thresh)
                # if win[0]>260 and win[2]<1450:
                if win[0] > 350 and win[2] < 1400:
                    stat.append(
                        [win[0]+1, win[2]+1]) if max_val[i] <= thresh else dyn.append([win[0]+1, win[2]+1])

            # after false positives are gone, a simpler if.
            if ((len(stat) > 10) or (len(stat) > len(dyn) and len(dyn) < 5)):
                ...
                if self.stat_greater_count > 10:
                    ...
                    self.medd = round(self.q_measure(M, [stat[0][1], stat[0][0], 1], [
                                      stat[-2][1], stat[-2][0], 1]), 3)
